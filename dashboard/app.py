...
def load_data():
    try:
        ...
        if os.path.exists(DB_PATH):
            ...
            by_rating = conn.execute("""
                ...
                FROM raw_prices GROUP BY rating ORDER BY avg_price DESC
            """).fetchdf()
            conn.close()
    ...
    books = [
        "The Death of Humanity", "Sapiens", "Sharp Objects",
        "Tipping the Velvet", "The Black Maria", "Soumission",
        "A Light in the Attic", "The Requiem Red", "Patience",
        "In Her Wake", "Obsidian", "Starving Hearts",
        "The Past Never Ends", "Masks and Shadows", "Birdsong",
        "The Secret of Dreadwillow", "The Electric Pencil",
        "On a Midnight Clear", "Our Band Could Be Your Life",
        "The Pioneer Woman Cooks"
    ]
    n = 100
    titles = [books[i % len(books)] + (f" Vol.{i//len(books)+1}" if i >= len(books) else "") for i in range(n)]
    prices = pd.DataFrame({
        "title": titles,
        "price_gbp": np.random.uniform(10, 58, n).round(2),
        "rating": np.random.choice(ratings, n),
        "availability": "In stock",
        "scraped_at": datetime.now().isoformat(),
    })
    by_rating = prices.groupby("rating").agg(
        avg_price=("price_gbp","mean"),
        book_count=("price_gbp","count"),
...
with col_left:
    ...
    fig = px.histogram(prices, x="price_gbp", nbins=20,
...
with col_right:
    ...
    fig2 = px.bar(by_rating, x="rating", y="avg_price",
...
st.info("Alerts fire automatically when any product price changes more than 5% between scrape runs. Scraper runs every 6 hours via GitHub Actions.")
...
filtered = prices[(prices["rating"].isin(rating_filter)) & (prices["price_gbp"] <= max_price)]
st.dataframe(filtered[["title","price_gbp","rating","availability","scraped_at"]],
